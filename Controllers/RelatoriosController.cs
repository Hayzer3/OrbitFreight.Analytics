    ...
    public class RelatoriosController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetRankingRotas()
        {
            var ranking = await _context.Alertas
                .Include(a => a.Carga)
                    .ThenInclude(c => c.Rota)
                .GroupBy(a => new { a.Carga.Rota.IdRota, a.Carga.Rota.Origem, a.Carga.Rota.Destino })
                .Select(g => new
                {
                    RotaId = g.Key.IdRota,
                    Trajeto = $"{g.Key.Origem} -> {g.Key.Destino}",
                    TotalAlertas = g.Count(),
                    AlertasCriticos = g.Count(a => a.NivelRisco == "ALTO")
                })
        ...
        public async Task<ActionResult<DashboardResumoDto>> GetDashboardResumo()
        {
            ...
            var cargasAtivas = await _context.Cargas.CountAsync(c => c.Status == "EM_TRANSITO");

            var alertasHoje = await _context.Alertas
                .Where(a => a.NivelRisco == "ALTO" && a.DataAlerta >= hoje)
                .CountAsync();

            var perdas = await _context.Alertas
                .Include(a => a.Carga).ThenInclude(c => c.TipoCarga)
                .Where(a => a.NivelRisco == "ALTO")
                .ToListAsync();

            var prejuizoTotal = perdas.Sum(a => a.Carga.TipoCarga.Nome.Contains("Vacina") ? 5000m : 1500m);

            var piorRota = await _context.Alertas
                .Include(a => a.Carga).ThenInclude(c => c.Rota)
                .GroupBy(a => new { a.Carga.Rota.Origem, a.Carga.Rota.Destino })
        ...
        public async Task<IActionResult> GetPerdasEstimadas()
        {
            var perdas = await _context.Alertas
                ...
                .Where(a => a.NivelRisco == "ALTO")
                .GroupBy(a => new { a.Carga.TipoCarga.IdTipo, a.Carga.TipoCarga.Nome })
                .Select(g => new
                {
                    TipoCargaId = g.Key.IdTipo,
                    Produto = g.Key.Nome,
